AJS.toInit(function ($) {
  ...
  mindmaps.ApplicationController = function() {
     ...
     */
    function doOpenDocument(loadableMindMap) {
        // close old document first
        var doc = mindmapModel.getDocument();
        doCloseDocument();
        //console.log(loadableMindMap);
        // var presenter = new mindmaps.OpenDocumentPresenter(eventBus, mindmapModel, new mindmaps.OpenDocumentView(), loadableMindMap);
        // presenter.go();
        var newDoc = mindmaps.Document.fromJSON(loadableMindMap);
        mindmapModel.setDocument(newDoc);
        return newDoc;
    }
    ...
    this.go = function(obj) {
      if(obj) {
        ...
        viewController.go();
        return doOpenDocument(obj);
      }
      return null;
    };
  
    this.getMap = function(doc) {
      mindmapModel.setDocument(doc);
      // console.log(mindmapModel.getDocument());
      return mindmapModel.getMindMap();
    }
